...
const ProfileScreen = () => {
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      {/* 👇 关键：用 ScrollView 包裹所有内容，实现滚动 + 底部避让 */}
      <ScrollView
        className="flex-1"
        // 底部加内边距，彻底避开 iOS 液态玻璃 TabBar
      ...
      >
        ...
        {/* SIGN OUT BTN */}
        <Pressable
          className="mt-6 mx-5 mb-5 flex-row items-center justify-center gap-2 rounded-xl border border-[#FF6B6B33] bg-surface px-4 py-4"
          onPress={async () => {
            try {
              await signOut()
            } catch (error) {
